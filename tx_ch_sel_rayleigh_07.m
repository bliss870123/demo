...
%% ************************************
ref_hex=dec2hex(0,8);%1=external ref 0=internal ref %% "8"為了適應網路傳輸資料的格式要求，轉換為8位16進制數
vco_cal_hex=dec2hex(1,8);%1=auxdac 0=reference clock %% "8"為了適應網路傳輸資料的格式要求，轉換為8位16進制數
aux_dac1_hex=dec2hex(0,8);
fdd_tdd_hex=dec2hex(1,8);%%1:FDD 2:TDD
trx_sw_hex=dec2hex(1,8);%%1:TX 2:RX
%% ************************************
bw_hex=dec2hex(20e6,8);
samp_hex=dec2hex(1e2,8); %e改成1M(Hz)
freq_hex=dec2hex(900e6,10); %%for ad9361  %改成900M(Hz)
tx_att1=dec2hex(200,8);
tx_att2=dec2hex(200,8);
tx_chan=3;%1=tx1;2=tx2;3=tx1&tx2
%% generate tone signal
%x=randi([0 127]); %0~127隨機產生一個數
%y=randi([0 127]);
nf1=60; %常態衰減值
nf2=75;
nf3=300;
...
%% **********************************************************
% 如果是單板收發測試，那麼在rx.m中沒必要再配置以下參數，如果只收不發，則在rx.m中要添加以下配置
...
pause(0.5);
%% send handshake2 cmd  (通知PS要發送的資料量)
...
% pause(10);
% %% send handshake2 cmd to stop adc thread (用以結束TX loop線程)
